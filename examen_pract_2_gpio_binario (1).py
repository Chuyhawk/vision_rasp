...
# ================================
# CONFIGURACIÓN GPIO HACIA UR / RASPBERRY
# ================================
# IMPORTANTE:
# La Raspberry trabaja a 3.3V.
# NO conectes GPIO directo a equipos industriales.
# Usa relevador / optoacoplador / interfaz adecuada.
#
# Pines actuales por pulso:
#   GPIO_OK    -> pulso cuando la decisión final es OK
#   GPIO_SCRAP -> pulso cuando la decisión final es SCRAP
#
# Pines nuevos por nivel lógico:
#   GPIO_DECISION_BINARIA:
#       1 = decisión final OK
#       0 = decisión final SCRAP
#       0 = también cuando aún no hay decisión final
#   GPIO_PRESENCIA_PIEZA:
#       1 = se detecta pieza
#       0 = no se detecta pieza

ENVIAR_SENAL_UR = True

GPIO_OK = 17                 # BCM
GPIO_SCRAP = 27              # BCM
GPIO_DECISION_BINARIA = 22   # BCM -> 1 OK | 0 SCRAP / sin decisión
GPIO_PRESENCIA_PIEZA = 23    # BCM -> 1 hay pieza | 0 no hay pieza
PULSO_SEG = 0.5
...
# ================================
# CLASE DE INTERFAZ DIGITAL HACIA UR
# ================================
...
class URDigitalBridge:
    def __init__(
        ...
        self.gpio_scrap = gpio_scrap
        self.gpio_decision = gpio_decision
        self.gpio_presencia = gpio_presencia

        self.last_pulse_result = None
        self.last_pulse_time = 0.0

        self.presencia_actual = None
        self.decision_actual = None

        if not self.enabled:
            print("[UR] Bridge digital deshabilitado.")
            return

        GPIO.setmode(GPIO.BCM)
        GPIO.setwarnings(False)

        GPIO.setup(self.gpio_ok, GPIO.OUT, initial=GPIO.LOW)
        GPIO.setup(self.gpio_scrap, GPIO.OUT, initial=GPIO.LOW)
        GPIO.setup(self.gpio_decision, GPIO.OUT, initial=GPIO.LOW)
        GPIO.setup(self.gpio_presencia, GPIO.OUT, initial=GPIO.LOW)

        self.reset_all()
        print("[UR] Bridge digital inicializado correctamente.")

    def reset_pulses(self):
        if not self.enabled:
            return
        GPIO.output(self.gpio_ok, GPIO.LOW)
        GPIO.output(self.gpio_scrap, GPIO.LOW)

    def set_final_decision(self, result=None):
        if not self.enabled:
            return

        if result == self.decision_actual:
            return

        # 1 = OK | 0 = SCRAP | 0 = sin decisión todavía
        if result == "OK":
            GPIO.output(self.gpio_decision, GPIO.HIGH)
            print("[GPIO] Decision binaria -> 1 (OK)")
        else:
            GPIO.output(self.gpio_decision, GPIO.LOW)
            if result == "SCRAP":
                print("[GPIO] Decision binaria -> 0 (SCRAP)")
            else:
                print("[GPIO] Decision binaria -> 0 (sin decision)")

        self.decision_actual = result

    def set_piece_presence(self, detected: bool):
        if not self.enabled:
            return

        detected = bool(detected)
        if detected == self.presencia_actual:
            return

        GPIO.output(self.gpio_presencia, GPIO.HIGH if detected else GPIO.LOW)
        print(f"[GPIO] Presencia de pieza -> {1 if detected else 0}")
        self.presencia_actual = detected

    def reset_states(self):
        if not self.enabled:
            return
        self.set_final_decision(None)
        self.set_piece_presence(False)
    ...
    def pulse_ok(self):
        if not self.enabled:
            return
        print("[UR] Pulso digital -> OK")
        GPIO.output(self.gpio_scrap, GPIO.LOW)
        GPIO.output(self.gpio_ok, GPIO.HIGH)
        time.sleep(PULSO_SEG)
        self.reset_pulses()

    def pulse_scrap(self):
        if not self.enabled:
            return
        print("[UR] Pulso digital -> SCRAP")
        GPIO.output(self.gpio_ok, GPIO.LOW)
        GPIO.output(self.gpio_scrap, GPIO.HIGH)
        time.sleep(PULSO_SEG)
        self.reset_pulses()

    def pulse_result(self, result: str):
        ahora = time.time()

        if result not in ("OK", "SCRAP"):
            return

        if (result == self.last_pulse_result) and ((ahora - self.last_pulse_time) < COOLDOWN_SENAL):
            return
        ...
        self.last_pulse_result = result
        self.last_pulse_time = ahora
...
def modo_en_vivo(detector, matcher, patron_bgr, ur_bridge):
    ...
    ur_bridge.set_piece_presence(False)
    ur_bridge.set_final_decision(None)
    ...
    while True:
        ...
        if resultado_estable in ("OK", "SCRAP"):
            ur_bridge.set_piece_presence(True)
            frames_sin_pieza = 0
            ...
            if not decision_tomada:
                ...
                if len(muestras_pieza) >= VENTANA_DECISION:
                    ...
                    ur_bridge.set_final_decision(decision_final)
                    ur_bridge.pulse_result(decision_final)
            ...
            if frames_sin_pieza >= FRAMES_SIN_PIEZA_RESET:
                ...
                ur_bridge.set_final_decision(None)
                ur_bridge.set_piece_presence(False)
                ur_bridge.last_pulse_result = None
                ur_bridge.last_pulse_time = 0.0
        ...
        presencia_txt = 1 if ur_bridge.presencia_actual else 0
        cv2.putText(img_kp, f"GPIO Presencia: {presencia_txt}", (10, 115),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 180, 0), 2)

        if decision_final is not None:
            decision_bin = 1 if decision_final == "OK" else 0
            cv2.putText(img_kp, f"Decision final: {decision_final} | GPIO Decision: {decision_bin}", (10, 145),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 0), 2)
        else:
            cv2.putText(img_kp, "Decision final: --- | GPIO Decision: 0", (10, 145),
...
def modo_foto_a_foto(detector, matcher, patron_bgr, ur_bridge):
    ...
    while True:
        ...
        if res in ("OK", "SCRAP"):
            ur_bridge.set_piece_presence(True)
            ur_bridge.set_final_decision(res)
            ur_bridge.pulse_result(res)
        else:
            ur_bridge.set_piece_presence(False)
            ur_bridge.set_final_decision(None)
            print("[UR] No se envía señal porque no se detecta pieza estable.")
...
def main():
    print("=" * 60)
    print("  SISTEMA DE VISION + SALIDA DIGITAL HACIA UR")
    print("=" * 60)
    print(f"  Algoritmo             : {ALGORITMO}")
    print(f"  Umbral OK             : >= {UMBRAL_MATCHES} matches")
    print(f"  Ratio                 : {UMBRAL_RATIO}")
    print(f"  KP min escena         : {UMBRAL_KP_ESCENA}")
    print(f"  Salida a UR           : {'ACTIVA' if ENVIAR_SENAL_UR else 'DESACTIVADA'}")
    print(f"  GPIO pulso OK         : {GPIO_OK}")
    print(f"  GPIO pulso SCRAP      : {GPIO_SCRAP}")
    print(f"  GPIO decision binaria : {GPIO_DECISION_BINARIA}")
    print(f"  GPIO presencia pieza  : {GPIO_PRESENCIA_PIEZA}")
    print("=" * 60)
    ...
    ur_bridge = URDigitalBridge(
        ...
        gpio_ok=GPIO_OK,
        gpio_scrap=GPIO_SCRAP,
        gpio_decision=GPIO_DECISION_BINARIA,
        gpio_presencia=GPIO_PRESENCIA_PIEZA
    )
