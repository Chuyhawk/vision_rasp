...
# ================================
# LED_OK:
#   HIGH = decisión final OK
#   LOW  = no OK
#
# LED_SCRAP:
#   HIGH = decisión final SCRAP
#   LOW  = no SCRAP
#
# LED_PRESENCIA:
...
#
# IMPORTANTE:
# En Raspberry, HIGH = 3.3V y LOW = 0V.
# GPIO -> resistencia 220/330 ohms -> ánodo LED
# cátodo LED -> GND

ENVIAR_SENAL_UR = True

GPIO_OK = 17         # BCM | LED 1
GPIO_SCRAP = 22      # BCM | LED 2
GPIO_PRESENCIA = 27  # BCM | LED 3
...
class URDigitalBridge:
    def __init__(self, enabled=True, gpio_ok=17, gpio_scrap=22, gpio_presencia=27):
        ...
        print(f"[GPIO] LED_OK        -> BCM {self.gpio_ok} | HIGH=OK")
        print(f"[GPIO] LED_SCRAP     -> BCM {self.gpio_scrap} | HIGH=SCRAP")
        print(f"[GPIO] LED_PRESENCIA -> BCM {self.gpio_presencia} | HIGH=PIEZA")
...
def main():
    print("=" * 60)
    print("  SISTEMA DE VISION + OK / SCRAP / PRESENCIA EN RASPBERRY")
    print("=" * 60)
    ...
    print(f"  GPIO OK          : BCM {GPIO_OK}")
    print(f"  GPIO SCRAP       : BCM {GPIO_SCRAP}")
    print(f"  GPIO PRESENCIA   : BCM {GPIO_PRESENCIA}")
    print("=" * 60)
